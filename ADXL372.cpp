...
ADXL372class::ADXL372class(SPI *spiDevice, int csPin) : spi(spiDevice), m_csPin(csPin)
{
...
void ADXL372class::writeRegister(uint8_t regAddress, uint8_t value)
{
...
void ADXL372class::ADXL372class::reset()
{
    writeRegister(RESET, 0x52);
}

void ADXL372class::ADXL372class::setOdr(Odr odr)
{
...
void ADXL372class::ADXL372class::setBandwidth(Bandwidth bandwidth)
{
...
void ADXL372class::ADXL372class::readAcceleration(float &x, float &y, float &z)
{
    byte status;
    do
    {
...
void ADXL372class::ADXL372class::readAccelerationY(float &y)
{
    byte status;
    do
    {
...
void ADXL372class::ADXL372class::readAccelerationZ(float &z)
{
    byte status;
    do
    {
...
{
    // Wait until the FIFO full flag is set
    byte status;
    do
    {
        status = readRegister(STATUS); // Use STATUS instead of a magic number
    } while ((status & 0x04) == 0);    // FIFO full flag

    // Start the FIFO read operation
    digitalWrite(m_csPin, LOW);

    uint8_t regAddress = (FIFO_DATA << 1) | 1; // Set the read bit
    uint8_t txBuffer[1] = {regAddress};
    uint8_t rxBuffer[2] = {0};

    // Send the FIFO data register address
    spi->xfer(txBuffer, 1, 0x00, 1);
    ...
    for (int i = 0; i < m_sampleSize; i++)
    {
        ...
        spi->xfer(0x00, 1, &msb, 1);
        spi->xfer(0x00, 1, &lsb, 1);

        // Combine MSB and LSB into 12-bit data
        fifoData[i] = (msb << 4) | (lsb >> 4);
    }
...
void ADXL372class::checkStandbyMode()
{
    ...
    if (mode != STANDBY)
    {
        std::cout << "WARNING: Activity, Inactivity and FIFO can only be set while in standy mode" << std::endl;
    }
}